module JimmyAddr::arc_protocol {
    
    use std::signer;
    use JimmyAddr::arc_coin;
    use aptos_framework::managed_coin;
    use aptos_framework::coin;
    use aptos_framework::aptos_coin::AptosCoin;
    use aptos_framework::timestamp;

    const MODULE_ADMIN: address = @JimmyAddr;
    ...
    public entry fun lend<CoinType> (
        ...
        if(!exists<Ticket<CoinType>>(signer_addr)){
            let ticket = Ticket {
                borrow_amount: 0,
                lend_amount: 0,
                claim_amount: 0,
            ...
            ticket_data.lend_amount =  _amount;
        }
        else{
            let ticket_data = borrow_global_mut<Ticket<CoinType>>(signer_addr);
    ...
    public entry fun isClaimable<CoinType>(
        ...
        let ticket_data = borrow_global_mut<Ticket<CoinType>>(signer_addr);
        assert!(ticket_data.lend_amount > 0, TICKET_NOT_EXIST);

        (timestamp::now_seconds() - ticket_data.last_interact_time) >= 600
    }
    ...
    public entry fun claim(
        ...
        let signer_addr = signer::address_of(admin);        
        assert!(exists<Ticket<arc_coin::ARC_Coin>>(signer_addr), TICKET_NOT_EXIST);

        let ticket_data = borrow_global_mut<Ticket<arc_coin::ARC_Coin>>(signer_addr);
        assert!(ticket_data.lend_amount > 0, TICKET_NOT_EXIST);
        assert!(timestamp::now_seconds() - ticket_data.last_interact_time >= 600, TIME_ERROR_FOR_CLAIM);

        let reward_amount = (ticket_data.lend_amount - ticket_data.borrow_amount) * (timestamp::now_seconds() - ticket_data.last_interact_time) / (600 * 100);
